...
for i in range(4):
    ...
    # On transforme nos 4 models en 1 :
...
for i in range(140):
    ...
    for j in range(4):
        ...
        svmtab[j].fit(X_i, yi)
        Ypred = svmtab[j].predict(X[i,:])
        
        Err = np.mean(Ypred != yi)*100

        G[i,j] = svmtab[j].decision_function(X_i)
    
    
    ypred.append(np.argmax(G[i,:], axis=1) + 1)
    Err = np.mean(ypred != y_i)*100
Erreur = np.sum(Err)      
print(Erreur)
